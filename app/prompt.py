...
我会通过系统上下文给你:今天的日期、我已有的项目列表(含 id/identifier/hex_color/parent_project_id)、标签列表、近期任务列表(含 id/title/project_id/due_date/priority/done/labels/done_at/created/updated/description/repeat_after/repeat_mode)。
...
8. **上下文字段含义**:所有日期字段未设置时为 null(不是"0001 年")。`done_at` 是任务完成日期(仅 done 的任务有),`updated` 是最近改动时间——判断"最近做了什么"优先用 `done_at`/`updated`,不要把不相关的旧任务当近期。`labels` 是任务已挂的标签标题数组,可据此回答"我的 XX 相关任务"并在 query 里用 `labels` 过滤。`description` 可能被截断。
"""


def _norm_date(v):
    """RFC3339 → 'YYYY-MM-DD';None/空/Go 零值(0001-01-01)→ None。"""
...
def _clean_desc(v, limit=200):
    """Vikunja 的 description 是 HTML:去标签、压空白、截断;空→None。"""
    if not v:
        return None
    text = re.sub(r"<[^>]+>", " ", str(v))
...
def build_context(
    ...
    recent_tasks 含:id / title / project_id / due_date / priority / done /
    done_at / created / updated / labels / description / repeat_after / repeat_mode,
    让 LLM 能逐字复用 title、看完成时间判断"近期"、按标签/描述推理。
    projects 含 identifier / hex_color / parent_project_id,过滤掉虚拟视图(id<=0)。
    所有日期经 _norm_date 归一:未设(Go 零值 0001-01-01)→ null。
    """
    payload = {
        ...
        "projects": [
            ...
            for p in projects
            if (p.get("id") or 0) > 0  # 过滤「所有待办」等虚拟视图(id<=0)
        ],
        "labels": [
        ...
        "recent_tasks": [
            {
                ...
                "done_at": _norm_date(t.get("done_at")),
                "created": _norm_date(t.get("created")),
                "updated": _norm_date(t.get("updated")),
                "labels": _label_titles(t),
                "description": _clean_desc(t.get("description")),
                "repeat_after": t.get("repeat_after", 0) or 0,
                "repeat_mode": t.get("repeat_mode", 0) or 0,
            }
